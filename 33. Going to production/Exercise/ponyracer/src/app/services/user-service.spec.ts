...
describe('UserService', () => {
  ...
  it('should fetch the score history', () => {
    const expectedHistory = [
      { instant: '2017-08-03T10:40:00Z', money: 10000 },
      { instant: '2017-08-04T09:15:00Z', money: 9800 }
    ] as Array<ScoreHistoryModel>;

    let actualHistory: Array<ScoreHistoryModel> = [];
    const userService = TestBed.inject(UserService);
    userService.getScoreHistory().subscribe(history => (actualHistory = history));

    http.expectOne(`${environment.baseUrl}/api/money/history`).flush(expectedHistory);

    expect(actualHistory, 'The observable should emit the score history').toBeDefined();
    expect(actualHistory).toStrictEqual(expectedHistory);
  });
});
