...
@Component({
  selector: 'pr-live',
  imports: [Pony, PrettyDatePipe, NgbAlert, TranslocoDirective],
...
export class Live {
  ...
  // );
  protected readonly winners = computed(() => this.raceModel()?.poniesWithPosition.filter(pony => pony.position >= 100));

  // protected readonly betWon = computed<boolean | undefined>(() =>
  //   this.winners()?.some(p => p.id === this.raceModel()?.betPonyId)
  // );
  protected readonly betWon = computed(() => this.winners()?.some(pony => pony.id === this.raceModel()?.betPonyId));
