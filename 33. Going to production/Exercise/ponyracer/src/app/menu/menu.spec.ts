...
class MenuTester {
  ...
  readonly languageSelect = page.getByRole('combobox', { name: 'Language' });
}
...
describe('Menu', () => {
  ...
  it('should display a select box with the current language', async () => {
    const tester = new MenuTester();
    await tester.toggleButton.click();

    await expect.element(tester.languageSelect).toBeInTheDocument();

    await expect.element(tester.languageSelect).toHaveDisplayValue('en');
    await expect.element(tester.languageSelect.getByRole('option')).toHaveLength(2);
    await expect.element(tester.languageSelect.getByRole('option').nth(1)).toHaveTextContent('ne');
  });
  ...
  it('should change the language', async () => {
    ...
    await tester.languageSelect.selectOptions('ne');

    expect(i18nService.changeLanguage).toHaveBeenCalledWith('ne');
  });
});
