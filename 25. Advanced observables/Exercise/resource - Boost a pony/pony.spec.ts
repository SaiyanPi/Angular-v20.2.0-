...
describe('Pony', () => {
  ...
  it('should display a running pony', async () => {
    ...
    await expect.element(tester.image).toHaveAttribute('src', 'images/pony-purple-running.gif');
  });
  ...
  it('should display a boosted pony', async () => {
    ...
    const tester = new PonyTester(
      TestBed.createComponent(Pony, {
        bindings: [inputBinding('ponyModel', ponyModel), inputBinding('isRunning', isRunning), inputBinding('isBoosted', isBoosted)]
      })
    );

    // then we should have an image with a boosted pony
    await expect.element(tester.image).toHaveAttribute('src', 'images/pony-purple-rainbow.gif');
  });
});
