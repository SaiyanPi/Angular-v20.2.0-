...
function mockRaceResource(...values: Array<RaceModel>) {
  let index = 0;
  return (_raceId: () => number) =>
    rxResource({
      stream: () => of(values[Math.min(index++, values.length - 1)]).pipe(delay(5))
    });
}
...
class BetTester {
  ...
  readonly liveLink = page.getByRole('link', { name: 'Watch live!' });
...
describe('Bet', () => {
  ...
  it('should display a link to go to live', async () => {
    ...
    await expect.element(tester.liveLink).toBeVisible();
    await expect.element(tester.liveLink).toHaveAttribute('href', '/races/12/live');
  });
});
